...
const defaultNavItems: NavItem[] = [
  { id: 'default-home', icon: <DefaultHomeIcon className="w-6 h-6" />, label: 'Home' },
  { id: 'default-explore', icon: <DefaultCompassIcon className="w-6 h-6" />, label: 'Explore' },
  { id: 'default-notifications', icon: <DefaultBellIcon className="w-6 h-6" />, label: 'Notifications' },
];
...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-transparent text-foreground border overflow-hidden ${className}`}>
      <EtherealShadowHeroContentWrapper
        className="absolute inset-0 -z-10 rounded-lg"
        color="rgba(240, 240, 240, 0.2)"
        noise={{ opacity: 0.05, scale: 1 }}
      />
      <div className="relative z-0 flex items-center h-full w-full px-2">
        ...
        <div 
          ref={limelightRef}
          className={`absolute z-10 w-11 h-[5px] rounded-full bg-primary shadow-[0_8px_12px_-4px_hsl(var(--primary)/0.6)] ${
            isReady ? 'transition-[left] duration-400 ease-in-out' : ''
          } ${limelightClassName}`}
          style={{ left: '-999px' }}
        >
          <div className="absolute left-[-30%] top-0 w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-primary/30 to-transparent pointer-events-none" />
        </div>
      </div>
    </nav>
  );
